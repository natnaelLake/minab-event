...
const currentItem = ref({ name: "", description: "", tag_image: null });
...
const { onResult: categoryResult, refetch: refetchCategories } =
  useQueryComposable(GetAllCategories, {
    limit: 10,
    offset: 0,
    order_by: [{ created_at: "desc" }],
  });
const { mutate: createTag, loading: createTagLoader } =
  useMutationComposable(CreateSingleTag);
const { mutate: createCategory } = useMutationComposable(CreateSingleCategory);
const { mutate: deleteTag } = useMutationComposable(DeleteSingleTag);
const { mutate: deleteCategory } = useMutationComposable(DeleteSingleCategory);
const { mutate: updateTag } = useMutationComposable(UpdateSingleTag);
const { mutate: updateCategory } = useMutationComposable(UpdateSingleCategory);
tagResult((result) => {
  if (result.data) {
...
const handleFileChange = (e) => {
  ...
  currentItem.value.tag_image = file; // Capture the file
};
...
const saveItem = async () => {
  try {
    if (isEditing.value) {
      ...
      if (currentTab.value === "tags") {
        createTagLoader.value = true;
        // Handle image upload if a file is selected
        if (currentItem.value.tag_image) {
          try {
            const base64File = await readFilesAsBase64(
              currentItem.value.tag_image
            );
            const uploadImagesInput = { input: { files: base64File } };
            const { data } = await uploadImages(uploadImagesInput); // Assuming this is your image upload function
            const uploadedImageUrl = data.uploadImages.imageUrls[0];
            currentItem.value.tag_image = uploadedImageUrl; // Update the image URL
          } catch (error) {
            createTagLoader.value = false
        ...
        // Create tag after uploading the image
        await createTag({
          name: currentItem.value.name,
          description: currentItem.value.description,
          tag_image: currentItem.value.tag_image,
        });
          ...
          <div class="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <form @submit.prevent="saveItem">
              ...
              <div class="flex justify-end space-x-2">
                ...
                <button
                  type="submit"
                  class="w-20 py-3 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-lg shadow-md focus:outline-none transition-all duration-300 flex justify-center items-center"
                  :disabled="createTagLoader"
                  :class="{
                    'opacity-50 cursor-not-allowed': createTagLoader,
                  }"
                >
                  <VueSpinner
                    v-if="createTagLoader"
                    size="24"
                    color="#ffffff"
                  />
                  <span v-else>{{ modalButton }}</span>
                </button>
              </div>
            </form>
          </div>
