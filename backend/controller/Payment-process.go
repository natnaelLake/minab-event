...
func ProcessPaymentHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&reqBody); err != nil {
		http.Error(w, fmt.Sprintf(`{"error": "Failed to decode request: %v"}`, err), http.StatusBadRequest)
		return
	}
	...
	payload := map[string]interface{}{
		"amount":       paymentInput.TotalPrice * float64(paymentInput.Quantity), // total price calculation
		"email":        paymentInput.Email,
		"first_name":   paymentInput.FirstName,
		"last_name":    paymentInput.LastName,
		"phone_number": "0912345678", // placeholder number
		"tx_ref":       paymentInput.TxRef,
		"callback_url": "https://webhook.site/077164d6-29cb-40df-ba29-8a00e59a7e60", // Change callback to your actual URL
	...
	if err != nil {
		http.Error(w, fmt.Sprintf(`{"error": "Failed to encode request payload: %v"}`, err), http.StatusInternalServerError)
		return
	}
	fmt.Printf("Payload: %s\n", string(payloadBytes)) // Log payload data for debugging

	payloadReader := bytes.NewReader(payloadBytes)

	// Setup custom client to bypass TLS certificate verification
	client := &http.Client{
	...
	if res.StatusCode != http.StatusOK {
		...
		http.Error(w, fmt.Sprintf(`{"error": "Received non-200 response: %d: %s"}`, res.StatusCode, string(responseBody)), http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		http.Error(w, fmt.Sprintf(`{"error": "Failed to read response body: %v"}`, err), http.StatusInternalServerError)
		return
	}
	...
	if err := json.Unmarshal(responseBody, &response); err != nil {
		http.Error(w, fmt.Sprintf(`{"error": "Failed to decode response: %v"}`, err), http.StatusInternalServerError)
		return
	}
