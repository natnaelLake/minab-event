...
const deleteModal = ref({ show: false, eventId: null });
const { mutate: updateEventStatus } = useMutationComposable(UpdateEventStatus);
...
const fetchMyEvents = async () => {
  try {
    const { onResult, onError } = useQueryComposable(GetMyEvents, {
      limit: 10,
      offset: 0,
